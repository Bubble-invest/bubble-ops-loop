...
# /home/claude/bubble-ops-loop, so NOTHING was actually live. {{OPERATOR}}
...
#
# Requires:
#   - SSH alias `{{VPS_HOST}}` configured (passwordless to claude user)
...
# {{OPERATOR}} flagged 2026-05-25 (msg 3165): Morty must be able to redeploy
# itself when {{OPERATOR}}'s Mac is in the shop for 3 days.
ON_MORTY=0
