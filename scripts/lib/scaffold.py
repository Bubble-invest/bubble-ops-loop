...
Once activated (onboarding complete), I run a `/loop` self-paced: I choose my next wake each tick via CronCreate (CronList-dedupe first) — toward the next due layer when work is pending, a longer cadence when quiet, a one-shot for tomorrow 08:03 Paris once all 4 layers are done. Never a hardcoded hourly/20-min cron. **The box's clock is UTC, not Paris (board #850)** — I never hand-write a Paris HH:MM as the cron literal (`08:03 Paris` is NOT `3 8 * * *`; that fired a live market order 2h late). I derive the box-UTC cron via `scripts/arm-wake-cron.sh <Paris-HH:MM> [daily|one-shot]` (DST-safe) and CronCreate the expression it prints, sanity-checking with `TZ=Europe/Paris date` at tick start. **The CronCreate prompt text itself is GENERATED, never authored by me** (board #1483/#1484): I run `due_missions.py wake-prompt --dept-dir .` and, if it exits 0, pass its stdout to CronCreate verbatim — I choose only WHEN (the cron time/cadence), never WHAT the prompt says, so an unsourced "operator said/flagged" note can never smuggle itself into a self-note and quietly narrow a mission (Ben's 2026-09-11 drift). It is FAIL-CLOSED by design: if it exits non-zero or prints nothing (my dept.yaml uses neither the Mac `loop.due_dispatch` schema nor a non-empty `recurring_missions` list it understands — board #1487 — or nothing live/due right now), I do NOT treat that as "nothing to do" — I CronCreate my own full tick protocol text for that one arm instead (the pre-#1484 fallback) and record one line naming the refusal in my HEARTBEAT ONLY, never Telegram (a refusal fires on every re-arm/compaction whenever nothing happens to be due; paging Joris over that would spam him).
At each tick:
