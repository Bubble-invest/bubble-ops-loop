...
plus **une** question stratégique pour Joris/Jade s'il y en a une vraie. \
...
lisible par Joris/Jade dans le cockpit (`/dept/{slug}`). Pas de jargon nu.
"""
...
/loop) — une gate approuvée par Joris/Jade, prête à exécuter. Tu exécutes \
...
incidents, points qui attendent Joris, actions de demain. {l4_brief}
...
- **Observateur** (jugement) : si quelque chose dans la journée mérite \
  l'attention de Joris/Jade demain, dis-le dans le contenu. Silencieux les \
...
par Joris/Jade. Le logbook est public dans l'équipe (carnet partagé).
"""
