...
| 2 | Console (cockpit) | `console/deploy/bubble-ops-console.service.template` + `scripts/deploy-console-to-vps.sh` | yes | Tailscale-served `:8443`. Fresh box: run `deploy-console-to-vps.sh` directly (materializes the unit, no pre-existing service needed). Ongoing deploys: `scripts/deploy-console-to-morty.sh` (pull + restart) calls `deploy-console-to-vps.sh --no-restart` after every pull, so a template edit merged to `main` reaches the box on the next deploy — see board #1081. **Runs as the dedicated `bubble-console` system uid from the root-owned `/opt/bubble-ops-loop` infra clone, not `claude`** — one-time uid provisioning + rollout runbook: board #1463. `deploy-console-to-vps.sh` also applies (idempotently, as root, before any restart) a scoped `setfacl` ACL granting `bubble-console` r/w on `/home/claude/agents` — board #1463 follow-up, needed on top of the `claude`-group membership because dept dirs are often `0700`/`0755`. |
