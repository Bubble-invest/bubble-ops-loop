...
REARM_TURN = (
    ...
    "The CronCreate prompt is GENERATED, never authored by you (board #1483/#1484 — a "
    "self-written wake note is exactly how an uncited 'operator flagged spend — be "
    "cost-conscious' claim self-reinforced across ~36 wake prompts and silently dropped "
    "a mission deliverable for 12 days). Run `due_missions.py wake-prompt --dept-dir "
    "<your dept repo root>` — the same generator your floor/backup tick already uses "
    "(it lives in your host's bubble-ops-loop checkout, e.g. "
    "`/opt/bubble-ops-loop/scripts/due_missions.py` on the VPS or the sibling "
    "`bubble-ops-loop` checkout next to your dept workspace on Mac; ask your "
    "wiki/HANDOFF if unsure). "
    "IF it exits 0: pass its ENTIRE stdout to CronCreate as the prompt, VERBATIM — "
    "never compose, paraphrase, edit, or append your own wording. Its output is "
    "guaranteed to be your full tick protocol text and NEVER a bare slash-command like "
    "/loop-now — that invariant is now enforced by the generator, not by your own "
    "drafting. "
    "IF it exits non-zero or prints nothing (FAIL-CLOSED by design — e.g. your "
    "dept.yaml uses neither the Mac loop.due_dispatch schema nor a non-empty "
    "recurring_missions list this generator understands, board #1487, or nothing "
    "is live/due right now): do NOT retry composing your own "
    "DUE_MISSIONS text and do NOT treat empty output as 'nothing to do' — instead "
    "CronCreate your own full tick protocol text (STEP A-F per CLAUDE.md, the "
    "pre-#1484 behavior) for THIS arm only, AND record one line in your HEARTBEAT "
    "ONLY (never Telegram — a refusal fires on every re-arm/compaction whenever "
    "nothing happens to be due, and paging Joris over that would spam him) naming "
    "the wake-prompt failure so the gap stays visible without paging anyone. "
