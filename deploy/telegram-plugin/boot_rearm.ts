...
export function bootRearmNotification(
  ...
  const content =
    ...
    "IF it exits non-zero or prints nothing (FAIL-CLOSED by design — e.g. your dept.yaml uses neither the Mac loop.due_dispatch schema nor a non-empty recurring_missions list this generator understands, board #1487, or nothing is live/due right now): do NOT retry composing your own DUE_MISSIONS text and do NOT treat empty output as 'nothing to do' — instead CronCreate your own full tick protocol text (STEP A-F per CLAUDE.md, the pre-#1484 behavior) for THIS arm only, AND record one line in your HEARTBEAT ONLY (never Telegram — a refusal fires on every re-arm/compaction whenever nothing happens to be due, and paging Joris over that would spam him) naming the wake-prompt failure (e.g. 'wake-prompt generator refused for this dept — used fallback text, see #1484/#1487') so the gap stays visible without paging anyone. " +
