...
are outside the collector's read set. Symlinks are ignored. Each source record
contains its repo-relative path, type, byte and line counts, and SHA-256. The
department record includes the Git HEAD when the input is a checkout. There is no
timestamp, so unchanged checkouts produce byte-for-byte equivalent JSON.
